...
function ImageDropzone({ onFilesAdded, onFileDelete, photos }: InputProps) {
    ...
    try {
      ...
      result.map((f) => {
        ...
        setImageStates((imageStates) => {
          ...
          const imageState = newFileStates.find(
  ...
  const _handleDelete = async (photoURL: string) => {
    ...
    const newState = imageStates.filter(
      (state) => state.downloadURL !== photoURL
    );
  ...
  return (
    <>
      ...
      >
        <div
          {...getRootProps()}
          className="text-center p-2 cursor-pointer border-2 border-dashed border-slate-300"
        >
          <input {...getInputProps()} />
          <div className="flex flex-col justify-center items-center text-xs text-gray-400">
            <UploadCloudIcon className="mb-3 h-8 w-8" />
            <div className="text-gray-400">Drag &amp; drop to upload </div>
            <div className="mt-2">
              <Button disabled={uploading} variant={"ghost"} type="button">
                Select
              </Button>
            </div>
          </div>
        ...
        {imageStates.map((image, index) => (
          <div
            key={index}
            className="border-0 p=0 w-full relative shadow-md bg-slate-200 rounded-md aspect-square h-full"
          >
            ...
            {uploading && image.state === "pending" && (
              <div
                className="absolute top-0 left-0 flex h-full w-full 
                        items-center justify-center rounded-md bg-black bg-opacity-70"
              >
                <Loader />
              </div>
            )}
